...
function assertAuthority(state: FilingState, actorId: string, scope: string): Authority {
  ...
  const authority = state.authorities.find(
    (candidate) =>
      ...
      Date.parse(candidate.effectiveAt) <= instant &&
      (!candidate.expiresAt || Date.parse(candidate.expiresAt) > instant),
  );
  if (!authority) throw new Error(`AUTHORITY_REQUIRED:${scope}`);
  return authority;
}

function audit(state: FilingState, actorId: string, operation: string, targetId: string, previousState?: string, resultingState?: string, authorityId?: string) {
  state.audit.push({ id: randomUUID(), actorId, operation, targetId, at: now(), previousState, resultingState, authorityId });
}
...
function computePackage(state: FilingState, item: FilingPackage) {
  ...
  const complete = required.filter((type) => documents.some((document) => document.type === type && ["VERIFIED", "SUBMITTED"].includes(document.status))).length;
  item.completionPercentage = required.length === 0 ? 100 : Math.floor((complete / required.length) * 100);
  if (item.status !== "SUBMITTED" && item.status !== "RECEIVED" && item.status !== "RETURNED" && item.status !== "ACTIVE") {
...
export async function getFilingOfficeSnapshot(memberView = false) {
  ...
  return {
    ...
    documents: state.documents.map(({ versions, ...document }) => ({ ...document, versions: versions.map(({ content: _content, ...version }) => version) })),
  ...
  switch (operation) {
    case "grantAuthority": {
      const authority: Authority = {
        id: randomUUID(),
        actorId: String(input.subjectActorId),
      ...
      audit(state, actorId, operation, authority.id);
      result = authority;
      break;
    }
    ...
    case "createBicApplicationPackage": {
      ...
      const missing = Object.entries(state.institution.prerequisites).filter(([, value]) => !value).map(([key]) => key);
    ...
    case "generateMonthlyBicPackage": {
      ...
      const item: FilingPackage = {
        id: randomUUID(), ownerType: "INSTITUTION", ownerId: state.institution.id, type: "MONTHLY_BIC_COLLATERAL_PACKAGE", status: "ASSEMBLING",
        requiredDocumentTypes: ["COLLATERAL_SCHEDULE", "BIC_1", "COLLATERAL_SCHEDULE_CONFIRMATION"], conditionalDocumentTypes: conditional,
        documentIds: [], completionPercentage: 0, submissionIds: [],
      };
      state.packages.push(item);
      audit(state, actorId, operation, item.id, undefined, item.status, authority.id);
      result = item;
    ...
    case "regenerateDocumentVersion": {
      ...
      const existing = state.documents.find((document) => document.packageId === packageItem.id && document.type === input.documentType);
      const content = String(input.content || "");
      if (existing) {
        if (["VERIFIED", "SUBMITTED"].includes(existing.status)) throw new Error("FROZEN_DOCUMENT_REQUIRES_CORRECTION_COPY");
        existing.versions.push({ version: existing.versions.length + 1, content, checksum: checksum(content), createdAt: now(), createdBy: actorId, frozen: false });
        ...
        const document: FilingDocument = {
          id: randomUUID(), ownerType: packageItem.ownerType, ownerId: packageItem.ownerId, packageId: packageItem.id,
          type: String(input.documentType), title: String(input.title || input.documentType), status: "GENERATED",
          ...
          versions: [{ version: 1, content, checksum: checksum(content), createdAt: now(), createdBy: actorId, frozen: false }],
        };
        state.documents.push(document); packageItem.documentIds.push(document.id); computePackage(state, packageItem);
        audit(state, actorId, operation, document.id, undefined, document.status, authority.id); result = document;
      }
      break;
    }
    case "requestSignature": {
      ...
      const previous = document.status; document.status = "AWAITING_SIGNATURE";
      audit(state, actorId, operation, document.id, previous, document.status, authority.id); result = document;
      break;
    }
    case "recordSignature": {
      ...
      const previous = document.status; document.status = "SIGNED"; document.signedBy = actorId;
    ...
    case "verifyDocument": {
      ...
      const previous = document.status; document.status = "VERIFIED"; document.verifiedBy = actorId; document.versions.at(-1)!.frozen = true;
      const packageItem = state.packages.find((candidate) => candidate.id === document.packageId); if (packageItem) computePackage(state, packageItem);
      audit(state, actorId, operation, document.id, previous, document.status, authority.id); result = document;
      break;
    }
    case "addCollateralRecord": {
      const authority = assertAuthority(state, actorId, "COLLATERAL_ADD");
      const amount = Number(input.amount); if (!Number.isFinite(amount) || amount <= 0) throw new Error("INVALID_COLLATERAL_AMOUNT");
      const record: CollateralRecord = { id: randomUUID(), institutionId: state.institution.id, description: String(input.description), amount,
        status: input.exception ? "EXCEPTION" : "PLEDGED", electronic: Boolean(input.electronic), creditCardReceivable: Boolean(input.creditCardReceivable), thirdPartyCustodian: Boolean(input.thirdPartyCustodian), createdAt: now() };
      state.collateral.push(record); audit(state, actorId, operation, record.id, undefined, record.status, authority.id); result = record; break;
    }
    case "withdrawCollateralRecord": {
      const authority = assertAuthority(state, actorId, "COLLATERAL_WITHDRAW");
      const record = state.collateral.find((candidate) => candidate.id === input.collateralId); if (!record) throw new Error("COLLATERAL_NOT_FOUND");
      const previous = record.status; record.status = "WITHDRAWN"; record.withdrawnAt = now();
      audit(state, actorId, operation, record.id, previous, record.status, authority.id); result = record; break;
    }
    case "generateCollateralSchedule": {
      const authority = assertAuthority(state, actorId, "DOCUMENT_GENERATE");
      const total = state.collateral.filter((item) => item.status === "PLEDGED").reduce((sum, item) => sum + item.amount, 0);
      result = { total, records: state.collateral.filter((item) => item.status === "PLEDGED") };
      audit(state, actorId, operation, String(input.packageId), undefined, String(total), authority.id); break;
    }
    case "exportSubmissionPackage": {
      const authority = assertAuthority(state, actorId, "PACKAGE_EXPORT");
      const packageItem = state.packages.find((candidate) => candidate.id === input.packageId); if (!packageItem) throw new Error("PACKAGE_NOT_FOUND");
      computePackage(state, packageItem); if (packageItem.status !== "READY_FOR_SUBMISSION") throw new Error("PACKAGE_NOT_READY");
      const manifest = state.documents.filter((document) => packageItem.documentIds.includes(document.id)).map((document) => ({ documentId: document.id, version: document.versions.at(-1)!.version, checksum: document.versions.at(-1)!.checksum }));
      audit(state, actorId, operation, packageItem.id, packageItem.status, packageItem.status, authority.id);
      result = { packageId: packageItem.id, destination: "Federal Reserve Bank of San Francisco — 12th District", manifest }; break;
    }
    case "recordSubmission": {
      const authority = assertAuthority(state, actorId, "PACKAGE_SUBMIT");
      const packageItem = state.packages.find((candidate) => candidate.id === input.packageId); if (!packageItem) throw new Error("PACKAGE_NOT_FOUND");
      computePackage(state, packageItem); if (packageItem.status !== "READY_FOR_SUBMISSION" && packageItem.status !== "RETURNED") throw new Error("PACKAGE_NOT_READY");
      const manifest = state.documents.filter((document) => packageItem.documentIds.includes(document.id)).map((document) => ({ documentId: document.id, version: document.versions.at(-1)!.version, checksum: document.versions.at(-1)!.checksum }));
      const submission: SubmissionRecord = { id: randomUUID(), packageId: packageItem.id, destination: "Federal Reserve Bank of San Francisco — 12th District", manifest, submittedAt: now(), submittedBy: actorId, status: "SUBMITTED" };
      state.submissions.push(submission); packageItem.submissionIds.push(submission.id); const previous = packageItem.status; packageItem.status = "SUBMITTED";
      state.documents.filter((document) => packageItem.documentIds.includes(document.id)).forEach((document) => { document.status = "SUBMITTED"; document.versions.at(-1)!.frozen = true; });
      audit(state, actorId, operation, packageItem.id, previous, packageItem.status, authority.id); result = submission; break;
    }
    case "recordReceipt":
    case "recordReturn":
    case "recordAcceptance": {
      ...
      if (submission) { submission.status = operation === "recordReceipt" ? "RECEIVED" : operation === "recordReturn" ? "RETURNED" : "ACCEPTED"; submission.reason = input.reason ? String(input.reason) : undefined; }
      audit(state, actorId, operation, packageItem.id, previous, packageItem.status, authority.id); result = packageItem; break;
    }
    default:
