...
export async function POST(request: NextRequest) {
  try {
    ...
    const updated = await NotificationService.archive(operator.institutionKey, operator.userId, notificationIds);
    return NextResponse.json({ updated });
  } catch (error) {
    console.error("NOTIFICATION_ARCHIVE_FAILED", error);
